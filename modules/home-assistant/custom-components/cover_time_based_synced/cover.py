...
SWITCH_COMMAND_GRACE_SECONDS = 3
...
def devices_from_config(domain_config):
    ...
    for device_id, config in domain_config[CONF_DEVICES].items():
        ...
        source_entity_id = config.pop(CONF_SOURCE_ENTITY_ID, None)
        device = CoverTimeBased(device_id, name, travel_time_down, travel_time_up, open_switch_entity_id, close_switch_entity_id, send_stop_at_ends, source_entity_id)
...
class CoverTimeBased(CoverEntity, RestoreEntity):
    def __init__(self, device_id, name, travel_time_down, travel_time_up, open_switch_entity_id, close_switch_entity_id, send_stop_at_ends, source_entity_id=None):
        ...
        self._last_switch_command_time = 0
        ...
        self._unsubscribe_source_listener = None

        self.tc = TravelCalculator(self._travel_time_down, self._travel_time_up)

    def _is_within_switch_grace(self):
        """Check if we're within the grace period after a switch command."""
        return (time.monotonic() - self._last_switch_command_time) < SWITCH_COMMAND_GRACE_SECONDS

    def _record_switch_command(self):
        """Record the time of a switch command."""
        self._last_switch_command_time = time.monotonic()

    async def _handle_source_state_changed(self, event):
        """Handle state changes from the source entity (e.g. Matter cover).

        When the source entity changes state externally (physical button),
        start/stop the travel calculator to track position based on time.

        The Bilresa has no endstops and current_position is unreliable (always 50).
        We only use the state transitions to detect button press/release:
        - closing/opening = button held, shade moving
        - open/closed = button released, shade stopped
        """
        ...
        if self._is_within_switch_grace():
            _LOGGER.debug(self._name + ': source entity changed within grace period, ignoring: %s -> %s', old_state_str, new_state_str)
            return

        _LOGGER.debug(
            self._name + ': source entity changed externally: %s -> %s',
            old_state_str, new_state_str
        )

        if new_state_str == "opening":
            if not self.tc.is_traveling():
                self._target_position = 100
                self.tc.start_travel_up()
                self.start_auto_updater()
                _LOGGER.debug(self._name + ': started tracking external opening from pos %d', self.tc.current_position())
        elif new_state_str == "closing":
            if not self.tc.is_traveling():
                self._target_position = 0
                self.tc.start_travel_down()
                self.start_auto_updater()
                _LOGGER.debug(self._name + ': started tracking external closing from pos %d', self.tc.current_position())
        elif new_state_str in ("open", "closed"):
            if self.tc.is_traveling():
                self.tc.stop()
            self.stop_auto_updater()
            _LOGGER.debug(self._name + ': external movement ended at %s, position now %d', new_state_str, self.tc.current_position())
    ...
    async def async_added_to_hass(self):
        ...
        if self._source_entity_id is not None:
            _LOGGER.debug(self._name + ': listening to source entity %s', self._source_entity_id)
            self._unsubscribe_source_listener = async_track_state_change_event(
                self.hass, [self._source_entity_id], self._handle_source_state_changed
            )
    ...
    def extra_state_attributes(self):
        ...
        if self._source_entity_id is not None:
            attr["source_entity_id"] = self._source_entity_id
        return attr
    ...
    async def _async_handle_command(self, command, *args):
        """We have cover.* triggered command. Reset known_position processing and execute"""
        self._processing_known_position = False
        self._record_switch_command()
